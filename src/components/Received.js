...
class ConnectedReceived extends Component {
	constructor(props) {
		...
		this.state = {
			monthFilter: [],
			amountFilter: { label: "Ontvangen", value: "Ontvangen" },
			statusFilter: true,
    	searchFilter: "",
			selected: [],
			selFilter: false
	...
	onDownload(rowsRaw, selected) {
			...
			downloadCsv(rowsFiltered, [2,3,5,6], ["Datum", "Bedrag", "Naam", "Omschrijving"], "ontvangsten Mobly");
	}

	render() {
		const hasError = (this.props.connectionError || !this.props.isConnected);
		const hasData = (!hasError && this.props.received);
		if (!this.props.received && !hasError) {
		...
		const rowsRaw = (hasData
				...
				receivedRows(this.props.received)
				: null;
